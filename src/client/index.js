...
const Chat = {
  ...
  init: function (name) {
    ...
    this.socket.emit('login', {name: name, userId: sessionStorage.getItem('userId')});
    // 监听告诉全部人是否有人登录
  ...
  submit: function (mes) {
    // 发送给服务端一个新增信息的事件
    this.socket.emit('message', { message:mes, name:sessionStorage.getItem('name'), userId: sessionStorage.getItem('userId'), date: getNowTime() });
  }
};
